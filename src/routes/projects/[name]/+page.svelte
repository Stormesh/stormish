...
{#snippet projectDescription()}
	<ProjectDescription text={data.project?.lgDescription || data.project?.description || data.project?.text} />
{/snippet}

<h1 class="text-center text-4xl font-extrabold">{data.project?.text}</h1>
<div class="relative m-5 bg-transparent rounded-xl">
	{#if data.project}
		<img
		...
		/>
		<ProjectDetails heading="Description" text={projectDescription} />
		{#if data.project?.technologies}
			<ProjectDetails heading="Technologies" text={projectCards} />			
		{/if}
		{#if data.project?.url}
